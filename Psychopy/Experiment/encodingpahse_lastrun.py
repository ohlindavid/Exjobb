...
expName = 'encodingpahse'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Oskar\\Documents\\GitHub\\exjobb\\Experiment setup\\encodingpahse_lastrun.py',
...
text2 = visual.TextStim(win=win, name='text2',
    ...
    languageStyle='LTR',
    depth=-3.0);
Association = visual.RatingScale(win=win, name='Association', marker='triangle', size=1.0, pos=[0.0, -0.4], low=1, high=3, labels=['Very difficult ', ' OK ', ' Very easy'], scale='Rate the assiciation between image and word')

# Create some handy timers
globalClock = core.Clock()  # to track the time since experiment started
routineTimer = core.CountdownTimer()  # to track time remaining of each (non-slip) routine 

# ------Prepare to start Routine "Instructionm"-------
continueRoutine = True
routineTimer.add(5.000000)
# update component parameters for each repeat
# keep track of which components have finished
InstructionmComponents = [text_2]
for thisComponent in InstructionmComponents:
...
InstructionmClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "Instructionm"-------
while continueRoutine and routineTimer.getTime() > 0:
    # get current time
    t = InstructionmClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=InstructionmClock)
...
trials = data.TrialHandler(nReps=30, method='sequential', 
    ...
    trialList=data.importConditions('conditionsTest.csv', selection='0:2'),
...
for thisTrial in trials:
    ...
    # update component parameters for each repeat
    text.setText(texten)
    image.setImage(bilden)
    image2.setImage(bilden)
    text2.setText(texten)
    Association.reset()
    # keep track of which components have finished
    trialComponents = [text, image, image2, text2, Association]
    ...
    while continueRoutine:
        ...
        # *Association* updates
        if Association.status == NOT_STARTED and t >= 10.5-frameTolerance:
            # keep track of start time/frame for later
            Association.frameNStart = frameN  # exact frame index
            Association.tStart = t  # local t and not account for scr refresh
            Association.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Association, 'tStartRefresh')  # time at next scr refresh
            Association.setAutoDraw(True)
        continueRoutine &= Association.noResponse  # a response ends the trial
    ...
    trials.addData('Association.response', Association.getRating())
    trials.addData('Association.rt', Association.getRT())
    trials.addData('Association.started', Association.tStart)
    trials.addData('Association.stopped', Association.tStop)
