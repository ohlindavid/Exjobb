...
class MorletConv(keras.layers.Layer):
    def __init__(self, input_dim,input_shape=[1024,25,1]):
        ...
        self.a = self.add_weight(name='a', shape=(self.etas,1), initializer=keras.initializers.RandomNormal(mean=10, stddev=0.0,seed=1), trainable=True)
        self.b = self.add_weight(name='b', shape=(self.etas,1), initializer=keras.initializers.RandomNormal(mean=7, stddev=0,seed=1), trainable=True)
